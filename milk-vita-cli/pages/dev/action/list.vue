    ...
    <div class="card">
        <div class="card-header">
            <div class="d-flex justify-content-between">
                ...
                <div class="btn-group">
                    <nuxt-link to="/dev/action/add" class="btn btn-primary">
                ...
                <form @submit.prevent="filter">
                    <div class="row">
                        <div class="col-md-4 form-group">
                            <select class="form-control" v-model="search.menu_id">
                                <option value="">মেনু নির্বাচন করুন</option>
                                <option v-for="(row, index) in all_menu" :key="index" :value="row.id">{{row.name_bn}}</option>
                            </select>
                        </div>
                        <div class="col-md-2">
                ...
                <table class="table table-bordered table-hover custom mb-2">
                    <thead>
                        <tr>
                            ...
                            <th>মেনুর নাম</th>
                            <th>অ্যাকশন মেনুর (BN)</th>
                            <th>অ্যাকশন মেনুর (EN)</th>
                            <th>ইউআরএল</th>
                    ...
                    <tbody>
                        <tr v-for="(row, index) in action_menus" :key="index">
                            ...
                            <td class="text-center">
                                <div class="btn-group custom">
                                    <nuxt-link :to="'/dev/action/edit?id='+row.id" class="btn btn-primary">
                        ...
                        <tr v-if="(action_menus.length <= 0)">
                            <th colspan="6">কোনো ডিপার্টমেন্ট পাওয়া যায়নি </th>
                        </tr>
                    </tbody>
                </table>
                <!-- ------------------ PAGINATION ------------------ -->
...
<script>
    ...
    export default {
        ...
        methods:{
            destroy(id){
                ...
                .then((result)=>{
                    if(result.isConfirmed){
                        this.http('action-menu/delete/'+id, 'delete')
                    }
                });
            },
            filter(){
            ...
            fetchAllMenu(){
                this.loader = true;
                this.http('action-menu/list', 'menu-list', {
        ...
        watch:{
            kernel(){
                for(const key in this.kernel){
                    if(key=='menu-list'){
                        this.action_menus = this.kernel[key].data;

    ...
    <div class="card">
        ...
        <div class="card-body min75vh">
                ...
                <table class="table table-bordered table-hover custom">
                    ...
                    <tbody>
                        ...
                        <tr v-for="(row, index) in challans" :key="index">
                            ...
                            <td>{{$en2bn(row.association_name)}}</td>
                            <td>{{$en2bn(row.amount_of_milk)}}&nbsp;লিটার</td>
                            <td>{{$shift(row.shift)}}</td>
                            <td>{{row.status}}</td>
                            <td class="text-center">
                                <div class="btn-group custom">
                                    <nuxt-link :to="'/plant-manager/challan/request/view?id='+row.id" v-if="$ck_action('/plant-manager/challan/request/view')">
            ...
            <div class="d-flex justify-content-end">
                <pagination v-model="page_no" :records="total_row" :per-page="per_page" @paginate="fetchChallanList"/>
            </div>
        </div>
    </div>
</template>
<script>
    ...
    export default {
        ...
        methods:{
            ...
            fetchChallanList(){
                this.loader = true;
                this.http('chilling-plant/challan/association', 'list', {
                    where: this.search
                });
            },

    ...
    <div class="card">
        <div class="card-header">
            <div class="d-flex justify-content-between align-items-center">
                ...
                <div class="btn-group">
                    <NuxtLink to="/processing-plant/product-stock/receive/list" class="btn btn-primary"  >
            ...
            <div class="card-body bg-white">
                ...
                <div class="table-responsive">
                    ...
                    <div class="text-right">
                        <button v-if="challan.is_receive==0" class="btn btn-success mt-2" @click="setApprove()">
                            <i class="fa fa-check" aria-hidden="true"></i>
                            <span>গ্রহণ করুন</span>
                        </button>
                        <button v-else class="btn btn-default mt-2 border">
                            <i class="fa fa-check" aria-hidden="true"></i>
                            <span>গ্রহণ করা হয়েছে</span>
                        </button>
                    </div>
                </div>
            </div>
...
export default {
    ...
    methods: {
        ...
        setApprove(){
            this.is_loader = true;
            this.http('stock-transfer/finish-product/receive-confirm/'+this.$route.query.id, 'approved');
        }
    },
    watch: {
        kernel: function () {
            for (const key in this.kernel) {
                if(key == "record") {
                    ...
                    if(this.kernel[key].errors) this.$toastr.w(this.$msgSanitize(this.kernel[key]));
                    else {
                        this.http('stock-transfer/finish-product/details/'+this.$route.query.id, 'record');
                        this.$toastr.s("সফলভাবে গ্রহণ করা হয়েছে");
                    }
                    this.is_loader = false;
                }
            }

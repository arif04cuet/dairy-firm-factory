    ...
    <div class="card">
        <div class="card-header">
            <div class="d-flex justify-content-between">
                ...
                <div class="btn-group">
                    <nuxt-link to="/raw-material/purchase/list" class="btn btn-primary">
                ...
                <table class="table table-bordered custom">
                    ...
                    <tbody>
                        <tr v-for="(row, index) in items" :key="index">
                            ...
                            <td width="130"><input @change="financialCalculation" @input="financialCalculation" v-model="items[index].price" type="number" min="0" step="any" class="form-control text-center"></td>
                            <td width="70">{{(items[index].quantity) * (items[index].price)}}</td>
                            <td width="30">
                        ...
                        <tr v-if="items.length>0">
                            <th class="text-right">সর্বমোট পণ্যের মূল্যঃ</th>
                            <td colspan="2" class="text-right">{{total_price * total_quantity}}</td>
                        </tr>
                    </tbody>
                    <!-- // -->
                    <tr v-if="items.length==0">
...
<script>
    export default {
        ...
        mounted(){
            ...
            // 
            this.http('raw-material/purchase/view/'+this.$route.query.id, 'editable');
            //
            this.http('product/category/list', 'categories', {
                select:['id', 'category_name'],
                where :{slug:'raw-materials'},
                children:'true'
            });
            this.getProductList();
        },
        methods:{
            itemDelete(id){
                ...
                .then((result) => {
                    if(result.isConfirmed) {
                        this.http('raw-material/purchase/item-delete/'+id, 'item-delete');
                    }
                })
            },
            submit(){
                if(this.$route.query.id){
                    ...
                    //
                    this.http('raw-material/purchase/update/'+this.$route.query.id, 'update', form);
                }
            },
            ...
            addRow(){
                if(this.search.product_id!='' && this.search.unit_id!=''){
                    this.http('raw-material/purchase/check-item', 'add-to-items', this.search);
                }
                ...
                for(const key in this.kernel){
                    ...
                    else if(key=='update')
                    {
                        if(this.kernel[key].errors){
                          ...
                          this.$router.push({path:'/raw-material/purchase/view?id='+this.kernel[key]});
                        }

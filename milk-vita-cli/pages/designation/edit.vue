...
<script>
    ...
    export default {
        ...
        methods:{
            submit(){
                var data = Object.assign({}, this.form);
                    ...
                    //
                this.http('designation/update/'+this.edit_id, 'store', data);
            },
            onChangeSystem(){
            ...
            childrenEntity(){
                this.http('entity/all', 'all_child_entities', {
                    where:{
                        parent_id : (this.form.entity_id != 0 ? this.form.entity_id : '0'),
                    }
                });
            },
        ...
        watch:{
            edit_id:function(){
                this.http('designation/list', 'designation', {
            ...
            kernel(){
                for(const key in this.kernel){
                    if(key=="designation"){
                        var response = Object.assign({}, this.kernel[key].data[0]);
                        this.form = this.kernel[key].data[0];
                        this.form.entity_id       = (response.parent_entity_id ? response.parent_entity_id : response.entity_id);
                        this.form.child_entity_id = (response.parent_entity_id ? response.entity_id : response.parent_entity_id);
                        // 
                        this.childrenEntity();
                    }
                        ...
                        if(this.kernel[key].errors){
                            ...
                            this.$toastr.s('পদবী সফলভাবে সংশোধন করা হয়েছে');

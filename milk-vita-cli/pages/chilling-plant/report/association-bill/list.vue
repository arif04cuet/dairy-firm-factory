    ...
    <div class="card">
        <div class="card-header">
            <div class="d-flex justify-content-between">
                ...
                <div class="btn-group">
                    <nuxt-link to="/chilling-plant/report/association-bill/generate" class="btn btn-primary" v-if="$ck_action('/chilling-plant/report/association-bill/generate')">
                        <i class="fa fa-plus"></i>&nbsp;নতুন রিপোর্ট জেনারেট করুন
                    </nuxt-link>
                </div>
            </div>
        </div>
        ...
        <div class="card-body min75vh">
            ...
            <div class="table-responsive" v-if="!loader">
                <table class="table table-bordered table-hover custom mb-2">
                    <thead>
                        <tr>
                            ...
                            <th class="text-center">রিপোর্টার তারিখ</th>
                            <th class="text-left">মোট টাকা</th>
                    ...
                    <tbody>
                        <tr v-for="(row, index) in vouchers" :key="index">
                            ...
                            <td class="text-left">{{ $en2bn(row.association_name) }}</td>
                            <td class="text-left">{{ row.association_code }}</td>
                            <td class="text-center">{{ $en2bn(row.from_date)+' হতে '+$en2bn(row.to_date) }}</td>
                            <td class="text-right">{{ number(row.grand_total) }}</td>
                            <td class="text-center">
                                <div class="btn-group custom">
                                    <nuxt-link :to="'/chilling-plant/report/association-bill/view?id='+row.id" v-if="$ck_action('/chilling-plant/report/association-bill/view')">
                                        <button class="btn btn-info">
                                            <i class="fa fa-eye" aria-hidden="true"></i>
                                        </button>
                                    </nuxt-link>
                                    <!-- // -->
                                    <nuxt-link :to="'/raw-material/purchase/edit?id='+row.id" v-if="$ck_action('/raw-material/purchase/edit')">
                                        <button class="btn btn-primary">
                                            <i class="fa fa-pencil-square-o" aria-hidden="true"></i>
                                        </button>
                                    </nuxt-link>
                                    <!-- // -->
                                    <nuxt-link to="#" v-if="$ck_action('/raw-material/purchase/destroy')">
                ...
                <div class="d-flex justify-content-end">
                    <pagination v-model="page_no" :records="total_row" :per-page="per_page" @paginate="getReport"/>
                </div>
...
<script>
    ...
    export default {
        ...
        methods:{
            ...
            getReport(){
                this.loader = true;
                this.http('chilling-plant/report/list', 'list', {
